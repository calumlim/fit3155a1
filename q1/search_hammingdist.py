...
#A class object which contains the functions to find the pattern in the text with a hammingdist<=1
class ZBox:
    ...
    #within the Z Array that has a hamming distance of <=1, it does this by
    ...
    def patternSearch(self, txt, rev_txt):
        ...
        hammingdist_arr = []
        #iterating through the array find for hamming distance <= 1
        for i in range(len(z_arr)):
            neg_ind = -i-start_ind+1
            if i+pat_len<len(z_arr):
                hammingdist = z_arr[i]+rev_z_arr[neg_ind]
                if hammingdist==pat_len:
                    hammingdist_arr.append([i+1, 1])
                elif hammingdist>pat_len:
                    hammingdist_arr.append([i+1, 0])
        return self.output_hammingdist(hammingdist_arr)

    #A function that iterates through the output array, then writes each line
    #into a file named output_hammingdist.txt
    def output_hammingdist(self, arr):
